...
public class ProductDBPostgres implements ProductDb {
    ...
    public Product get(int id) {
        ...
        String sql = "SELECT * from r0705829_schema.product where productId=" + id;
    ...
    public List<Product> getAll() {
        ...
        try(Connection connection = DriverManager.getConnection(url,properties);
            Statement statement = connection.createStatement()){
            ResultSet result = statement.executeQuery("SELECT * FROM r0705829_schema.product");
            while (result.next()){
    ...
    public void add(Product product){
        ...
        String sql = "INSERT INTO r0705829_schema.product VALUES(?,?,?,?);";
        try(Connection connection = DriverManager.getConnection(url, properties);
            //Statement statement = connection.createStatement()){
    ...
    public void update(Product product) {
        ...
        String sql = "UPDATE r0705829_schema.product SET name= ?, description = ?, price= ? WHERE productid=?;";
        try(Connection connection = DriverManager.getConnection(url, properties);
            //Statement statement = connection.createStatement()) {
            PreparedStatement statement = connection.prepareStatement(sql)){
    ...
    public void delete(String id) {
        try(Connection connection = DriverManager.getConnection(url,properties);
            Statement statement = connection.createStatement()){
            statement.execute("DELETE FROM r0705829_schema.product where productid='" + id+"'");
        }
        catch(SQLException e){
            throw new DbException(e);
        }
